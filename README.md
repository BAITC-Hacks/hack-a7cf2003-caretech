...
Для локального запуска без Go переключите в интерфейсе `Демо-режим`, чтобы увидеть подготовленный каталог. Демо-данные явно помечены и не называются актуальной корзиной EKT. API-режим включён по умолчанию; для явного демо можно задать `VITE_DEMO_MODE=true`. В API-режиме фронт не обращается к EKT API или LLM напрямую и не хранит ключи в браузере.
